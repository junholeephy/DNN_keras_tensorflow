void self_Pull_kin()
{
  ...
  TFile* f = new TFile("self_generative_tree_kin_200k_1k_256.root","READ");
...
//  HistoTarget_Test->Scale(scale);
  ...
  TH1F *htrain_pull = new TH1F("htrain_pull","htrain_pull",200,-1,1);
  TH1F *htest_pull  = new TH1F("htest_pull","htest_pull",200,-1,1);		htest_pull->SetLineColor(kRed);
  ...
  Double_t scale = norm/(htest_pull->Integral());
  htest_pull->Scale(scale);
  htest_pull->Draw("same");

//  htest_pull->Draw();
//  htrain_pull->Draw("same");

  TLegend* legend = new TLegend(0.2, 0.3, 0.4, 0.5, "");
  legend->SetFillColor(kWhite);
  legend->AddEntry(htrain_pull->GetName(), "kin_train", "l");
  legend->AddEntry(htest_pull->GetName(), "kin_test", "l");
  legend->Draw();
