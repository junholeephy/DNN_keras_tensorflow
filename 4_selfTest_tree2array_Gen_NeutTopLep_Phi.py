...
LOAD_MODEL_WEIGHTS = 'self_models_GEN/4_Gen_NeutTopLep_Phi/starter1_kin_model_weights_EN260000_LN1_E1000_NN50_B500_adam_LR1e-05_DR0.0.h5' 	#KIN
###############################################################################################################
...
mc_kin_ttz_weight_logmax_ = tree2array(tree, branches='mc_kin_ttz_weight_logmax')
###############################################################################################################
...
I1 = 0
for i1 in range(mc_kin_ttz_weight_logmax_.size):
        if(mc_kin_ttz_weight_logmax_[i1] > -600):
                I1 = I1 + 1
print("KIN's Total event number is : ", mc_kin_ttz_weight_logmax_.size)
print("KIN's Valid event number is : ",  I1)
num_Valid = np.zeros(I1)
I2 = 0
for i2 in range(mc_kin_ttz_weight_logmax_.size):
        if(mc_kin_ttz_weight_logmax_[i2] > -600):
                num_Valid[I2] = i2
                I2 = I2 + 1
###############################################################################################################
...
mc_kin_ttz_weight_logmax = np.zeros(I2)
...
for j1 in range(reco_bj1_Energy.size):
	...
	mHT[j1]			= mHT_[jj1]
	TARGET[j1]      = Gen_NeutTopLep_Phi_[jj1]
...
#	Gen_WTopLep_mW[j1]      	= Gen_WTopLep_mW_[jj1]
	...
	mc_kin_ttz_weight_logmax[j1] =	mc_kin_ttz_weight_logmax_[jj1]
...
if(MEM==True):
	np.savetxt("self_loss_values_GEN/4_Gen_NeutTopLep_Phi/loss_MEM_EN"+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+".txt", new_loss_history, delimiter=" ")
else:
	np.savetxt("self_loss_values_GEN/4_Gen_NeutTopLep_Phi/loss_KIN_EN"+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+".txt", new_loss_history, delimiter=" ")
...
if(MEM==True):
	modelRegress.save_weights('self_models_GEN/4_Gen_NeutTopLep_Phi/mem_model_weights_EN'+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+'.h5')
else:
	modelRegress.save_weights('self_models_GEN/4_Gen_NeutTopLep_Phi/kin_model_weights_EN'+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+'.h5')


if(MEM==True):
	f = TFile("self_trees_GEN/4_Gen_NeutTopLep_Phi/self_tree_mem_EN"+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+".root", "recreate")
else:
	f = TFile("self_trees_GEN/4_Gen_NeutTopLep_Phi/self_tree_kin_EN"+str(upper_limit)+"_LN"+str(ijkl)+"_E"+str(EPOCHS)+"_NN"+str(NODENUM)+"_B"+str(BATCH_SIZE_train)+"_"+str(OPTIMIZER)+"_L"+str(L2)+"_DR"+str(DROP_RATE)+".root", "recreate")
...
hist_target_train = TH1F('TrainData','TrainData',100,-100,100)
hist_target_test = TH1F('TestData','TestData',100,-100,100)
hist_output_train = TH1F('OutputDataTrain','OutputDataTrain',100,-100,100)
hist_output_test = TH1F('OutputDataTest','OutputDataTest',100,-100,100)
